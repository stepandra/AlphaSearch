defmodule ResearchJobs.Retrieval.TavilySearchAdapter do
  @moduledoc """
  Explicit TAVILY basic-search adapter.

  The adapter issues one straightforward Tavily search request via `Req`,
  ...
  def search(%SearchRequest{provider: :tavily} = request, opts) do
    provider_config = ProviderConfig.provider!(:tavily)
  ...
  def search(%SearchRequest{provider: provider}, _opts) do
    {:error,
     %ProviderError{
       ...
       message: "TAVILY adapter expected a :tavily request, got #{inspect(provider)}"
     }}
  end
  ...
  defp normalize_response(_request, %Req.Response{status: status, body: body})
    ...
    {:error,
     %ProviderError{
       ...
       message: response_message(body, "TAVILY search request failed with status #{status}"),
       raw_payload: body
     }}
  end
  ...
  defp results(body) do
    {:error,
     %ProviderError{
       ...
       message: "TAVILY response is missing a results list",
       raw_payload: raw_payload_subset(body)
     }}
  end
  ...
  defp normalize_hit(%{"title" => title, "url" => url} = result, request, fallback_rank)
    ...
    {:ok,
     %NormalizedSearchHit{
       ...
       title: title,
       url: url,
       snippet: optional_string(result["content"]),
       raw_payload: result
     }}
  end

  defp normalize_hit(_result, _request, fallback_rank) do
    {:error, "TAVILY result at index #{fallback_rank} is missing title or url"}
  end
  ...
  defp search_payload(%SearchRequest{query: query, max_results: max_results}) do
    ...
    if is_integer(max_results) and max_results > 0 do
      Map.put(payload, "max_results", min(max_results, @max_results))
    else
      payload
    end
  end
