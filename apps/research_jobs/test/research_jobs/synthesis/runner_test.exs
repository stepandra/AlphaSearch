defmodule ResearchJobs.Synthesis.RunnerTest do
  ...
  alias ResearchJobs.Synthesis.{ProviderError, Runner}
  ...
  test "runs synthesis end-to-end and persists a completed artifact" do
    ...
    assert run.artifact.content =~ "## Executive Summary"

    assert %ResearchCore.Synthesis.Artifact{} =
             ResearchStore.successful_synthesis_artifact(snapshot.id, "literature_review_v1")

    assert [%ResearchCore.Synthesis.Artifact{}] = ResearchStore.list_snapshot_reports(snapshot.id)
  end

  test "persists provider failures without fabricating an artifact" do
    snapshot = persisted_snapshot_fixture()
    ...
    assert run.provider_failure.reason == "timeout"
    assert run.artifact == nil
    assert ResearchStore.successful_synthesis_artifact(snapshot.id, "literature_review_v1") == nil
  end

  test "persists validation failures when provider output violates the explicit profile" do
    snapshot = persisted_snapshot_fixture()

    assert {:error, run} =
             Runner.run(snapshot.id, "literature_review_v1",
               provider: Fake,
               provider_opts: [content: invalid_markdown()]
             )

    assert run.state == :validation_failed
    assert %ResearchCore.Synthesis.ValidationResult{valid?: false} = run.validation_result

    assert Enum.any?(
             run.validation_result.citation_errors,
             &(&1["type"] == "unknown_citation_key" or &1.type == :unknown_citation_key)
           )

    assert run.artifact == nil

    assert %ResearchCore.Synthesis.ValidationResult{valid?: false} =
             ResearchStore.synthesis_validation_failures(run.id)
  end
  ...
  defp invalid_markdown do
    """
    ## Ranked Important Papers and Findings
    Unknown citation [REC_9999].

    ## Executive Summary
    Missing required order [REC_0001].
    """
  end
